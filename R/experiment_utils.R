...
#' @param .estParallel should each internal estimator be run in parallel
#' @param .estVerbose write the name of the internal estimator being tested
#'
...
run_one_experiment <- function(data, in_set_perc, form, 
                               ...
                               seed=1234, site_id="site", time="time", .estParallel = TRUE,
  ...
  if(.estVerbose) cat(paste0("\nRunning out-set gold error estimate...\n"))
  ...
  `%myfun%` <- ifelse(.estParallel, `%dopar%`, `%do%`)
  
  # perf.est <- list()
  perf.est <- foreach::foreach(j = 1:length(in_estimators), .inorder = TRUE) %myfun% {
  # for(j in 1:length(in_estimators)){
    # if(.verbose) cat(paste0(" Running ", in_est_nms[j], "... "))
    ...
    if(.estVerbose) cat(paste0(" Finished ", in_est_nms[j], ". "))
    
    res
    # perf.est[[ in_est_nms[j] ]] <- res
  }
  names(perf.est) <- in_est_nms
...
run_multiple_experiments <- function(data_list, in_set_perc, form, 
                               ...
                               .estParallel = FALSE, .estVerbose = FALSE, .compress = TRUE){
  
  lapply(data_list, function(dat){ 
    one_res <- run_one_experiment(dat, 
                                    ...
                                    .estParallel = .estParallel, .estVerbose = .estVerbose)
    if(.compress) one_res <- compressExp(one_res)
    one_res
    })
...
#' @param .expVerbose a Boolean indicating whether progress should be reported on during experiments
...
run_all_experiments <- function(models, nested_data_list, in_set_perc, form, 
                                ...
                                evaluator = "evaluate", eval.pars=NULL,
                                seed=1234, site_id="site", time="time",
                                .compress=FALSE, .progress=NULL, .expVerbose=TRUE, .saveMem=FALSE,
                                .estParallel = TRUE, .estVerbose=ifelse(.estParallel, FALSE, TRUE)){
